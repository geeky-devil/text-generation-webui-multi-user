...
def start_new_chat(state):
    ...
    if shared.args.username: unique_id=unique_id+"_"+shared.args.username
...
def find_all_histories(state):
    if shared.args.multi_user:
        character=state['character_menu']                                                                                       ####heere
        p = Path(f'logs/chat/{character}/{shared.args.username}')
        if p.exists():
            ...
            #histories = sorted(p, key=lambda x: x.stat().st_mtime, reverse=True)
